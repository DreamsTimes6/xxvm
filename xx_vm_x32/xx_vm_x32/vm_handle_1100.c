...
mov vm_sp,[vm_sp+00]
*/
int vm_handle_1100(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_mov,sop_len)==0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,vm_sp)==0 )
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0]==0x00)
				{
					...
					sprintf(outs+strlen(outs),"sp=[sp]=[%08x]=%08x  ",xx_contect[n].r[index],xx_contect[n+1].r[index]);
