...
movzx ax,byte ptr ds:[ecx]
lea ebp,dword ptr ss:[ebp+0x2]
mov word ptr ss:[ebp],ax

mov reg,[sp+00]
movzx wreg,b[reg]
add sp,02;lea sp,[sp+02]
mov w[sp+00],wreg
*/
int vm_handle_0502(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_mov,sop_len)==0)
		{
			...
			if(xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type==INST_ITEM_TYPE_REG)
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0]==0x00)
				{
					...
					sprintf(outs+strlen(outs),"(add sp,2) [sp]=%08x  ",xx_contect[n+1].r[index]);
	...
	{
		/*检查操作码*/
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_movzx,sop_len)==0 ||
			memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_mov,sop_len)==0)
		{
			/*检查第二个操作项类型*/
			if(xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type==INST_ITEM_TYPE_REG )
			{
				/*检查第一个操作项类型*/
				if(xx_inst[n].xx_inst_items.xx_inst_items_flag[1].item_type==INST_ITEM_TYPE_MREG &&
					xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def,t0)==0 )
				{
					if(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_data_size==INST_ITEM_SIZE_8)
					{
						...
						sprintf(outs+strlen(outs),"[[sp]]=[%08x]=",xx_contect[n].r[index]);
