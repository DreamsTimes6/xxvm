...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
shl w[sp],b[sp+2],w[sp+4]



*/
int vm_handle_0e02(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_shl,sop_len)==0)
		{
				...
				xreg_incmp(t0,xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def)==0)
			{
					...
					xreg_incmp(t1,xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def)==0)
				{
					...
					sprintf(outs+strlen(outs),"w[sp]=%08x  ",xx_contect[n].r[index]);
					...
					sprintf(outs+strlen(outs),"b[sp+2]=%08x  ",xx_contect[n].r[index]);
					...
					//sprintf(outs+strlen(outs),"%s=",tmp);
					index=get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					sprintf(outs+strlen(outs),"d[sp]=eflag,w[sp+4]=%08x  ",xx_contect[n+1].r[index]);
