...
int vm_handle_0e10(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_shld, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, t0) == 0)
			{
					...
					xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def, t1) == 0)
				{
					...
					{
						sprintf(outs + strlen(outs), "(sub sp,2)  ");
						...
						//sprintf(outs+strlen(outs),"%s=",tmp);
						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
						sprintf(outs + strlen(outs), "[sp]=rflag,[sp+8]=%llx", xx_contect[n + 1].r[index]);
						...
						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
						memcpy(item_data.var[0], &xx_contect[n].r[index], 4);
						memcpy(item_data.result, &xx_contect[n + 1].r[index], 4);

						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def);
						memcpy(item_data.var[1], &xx_contect[n].r[index], 4);
