...
int vm_handle_10503(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_mov, sop_len) == 0)
		{
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0] == 0x00)
				{
					...
					sprintf(outs + strlen(outs), "[sp]=%llx  ", xx_contect[n + 1].r[index]);
