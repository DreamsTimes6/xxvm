...
mov [sp+00],t0
*/
int vm_handle_0100(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_data_size==INST_ITEM_SIZE_32)
				{
					...
					if(xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_base_def,vm_regbase)==0 )
					{
						...
						{	
							...
							push_index=xx_contect[n].r[index];
	...
	if(reg_status_regbase.reg_status[push_index/4]==1)
	{
		...
		{
			memset(tmp,0,sizeof(tmp));
			xx_get_reg(reg_status_regbase.reg_def[push_index/4],tmp);
			sprintf(outs+strlen(outs),"%s",tmp);
