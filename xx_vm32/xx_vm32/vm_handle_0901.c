...
mov [sp+4],t0
*/
int vm_handle_0901(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
	...
	for(n=0;n<num;n++)
	{
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_not,sop_len)==0)
		{
				...
				xreg_incmp(t1,xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def)==0)
			{
				...
				memcpy(item_data.var[1], &xx_contect[n].r[get_reg_index(t1)], 2);
