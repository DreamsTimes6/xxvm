...
#define ENTRY_REG_NUM sizeof(entry_reg)/sizeof(struct VM_ENTRY_REG)

int vm_shandle_0000(struct XX_INST *xx_inst,int num,char *outs,int outs_size)
{
	int n=0;
	int m=0;
	int iret=0;
	...
	int tmp_regn=0;
	char tmp_reg[ENTRY_REG_NUM+1][20];
	
	for(n=0;n<ENTRY_REG_NUM;n++)
	{
		entry_reg[n].reg_seq=-1;
	}
