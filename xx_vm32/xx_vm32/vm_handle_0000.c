...
int vm_handle_0000(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int m=0;
	int iret=0;
	...
	//_Addtolist(0,1,"vm_shandle_0000");
	...
	for(n=0;n<num;n++)
	{
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_mov,sop_len)==0)
		{
			...
			if(xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type==INST_ITEM_TYPE_REG && 
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,vm_key)!=0)
			{
				/*检查第一个操作项类型*/
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def,vm_regbase)==0)
				{
					...
					memcpy(t0,xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,xx_inst[n].xx_inst_items.xx_inst_items_var[0].reg_def_len);
					f_cnd=1;
				}
			}
		}
	...
	for(n=0;n<num;n++)
	{
		...
		{
			/*检查第一个操作项类型*/
			if(xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type==INST_ITEM_TYPE_REG &&xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,vm_regbase)==0)
			{
				/*检查第二个操作项类型*/
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_mov,sop_len)==0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,vm_key)==0)
			{
				...
				if(xx_inst[n].xx_inst_items.xx_inst_items_flag[1].item_type==INST_ITEM_TYPE_REG )
				{
					...
					memcpy(t1,xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def,\
