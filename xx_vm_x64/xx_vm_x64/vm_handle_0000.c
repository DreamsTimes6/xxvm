...
int vm_handle_0000(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int m = 0;
	int iret = 0;
...
#ifdef DEBUG2_LOG
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "1 %llx\r\n", *(ulong64*)xx_inst[0].xx_inst_code.current_addr);
		xx_append_file(log_file, debug_buf, strlen(debug_buf));
#endif
		return -1;
	}
	...
	for (n = 0; n < num; n++)
	{
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_sub, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, vm_regbase) == 0)
			{
				...
				{
					/*检查第二个操作项寄存器*/
					if (*(unsigned int*)xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const == 0x00000180)
					{
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_lea, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, vm_regbase) == 0)
			{
				...
				if (xx_inst[n].xx_inst_items.xx_inst_items_flag[1].item_type == INST_ITEM_TYPE_MREGDIS)
				{
					...
					{
						if (*(unsigned int*)xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const == 0xfffffe80)
						{
							//_Addtolist(0,1,"second condition inst_num:[%d]",n);
							s_cnd = 1;
						}
