...
int vm_handle_0e11(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
	...
	sprintf(debug_buf, "0e11\r\n");
	xx_append_file(log_file, debug_buf, strlen(debug_buf));
#endif
	//_Addtolist(0,1,"vm_handle_0e11");
	...
	for (n = 0; n < num; n++)
	{
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_mov, sop_len) == 0)
		{
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
				...
				if (xx_inst[n].xx_inst_items.xx_inst_items_flag[1].item_type == INST_ITEM_TYPE_MREGDIS &&
					xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_base_def, vm_sp) == 0 &&
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0] == 0x10 &&
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_data_size == INST_ITEM_SIZE_8)
				{
	...
	{
		/*检查操作码*/
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_shld, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, t0) == 0)
			{
				...
				{
					/*检查第三个操作项类型*/
					if (xx_inst[n].xx_inst_items.xx_inst_items_flag[2].item_type == INST_ITEM_TYPE_REG &&
						xreg_incmp(t2, xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def) == 0)
					{
						sprintf(outs + strlen(outs), "(add sp,2)  ");
						...
						//memset(tmp,0,sizeof(tmp));
						//xx_get_reg_mic(xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def,tmp);
						//sprintf(outs+strlen(outs),"%s=",tmp);
						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def);
						sprintf(outs + strlen(outs), "%llx  ", xx_contect[n].r[index]);
						...
						//sprintf(outs+strlen(outs),"%s=",tmp);
						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
						sprintf(outs + strlen(outs), "q[sp]=eflag,q[sp+8]=%llx", xx_contect[n + 1].r[index]);
						...
						//index=get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
						//sprintf(outs+strlen(outs),"[sp]=eflag,[sp+4]=hdword()",xx_contect[n+1].r[index]);
						o_cnd = 1;

						item_data.item_num = 3;
						...
						memcpy(item_data.result, &xx_contect[n + 1].r[index], sizeof(xx_contect[n + 1].r[index]));

						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def);
						memcpy(item_data.var[1], &xx_contect[n].r[index], sizeof(xx_contect[n].r[index]));


						index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def);
						memcpy(item_data.var[2], &xx_contect[n].r[index], 1);
