...
int vm_handle_10e02(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
	...
	sprintf(debug_buf, "0e02\r\n");
	xx_append_file(log_file, debug_buf, strlen(debug_buf));
#endif
	//_Addtolist(0,1,"vm_handle_0e00");
	...
	{
		/*检查操作码*/
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_shl, sop_len) == 0)
		{

			/*检查第一个操作项类型*/
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG && \
				xreg_incmp(t0, xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def) == 0 && \
				xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_data_size == INST_ITEM_SIZE_8)
			{
				/*检查第二个操作项类型*/
				...
				{
					sprintf(outs + strlen(outs), "(sub sp,6)  ");
					...
					sprintf(outs + strlen(outs), "b[sp+2]=%llx  ", xx_contect[n].r[index]);
					...
					sprintf(outs + strlen(outs), "b[sp]=%llx  ", xx_contect[n].r[index]);
					...
					//sprintf(outs+strlen(outs),"%s=",tmp);
					index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					sprintf(outs + strlen(outs), "q[sp]=eflag,b[sp+8]=%llx  ", xx_contect[n + 1].r[index]);
					...
					index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					memcpy(item_data.var[0], &xx_contect[n].r[index], 1);

					memcpy(item_data.result, &xx_contect[n + 1].r[index], 1);

					index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def);
					memcpy(item_data.var[1], &xx_contect[n].r[index], 1);
