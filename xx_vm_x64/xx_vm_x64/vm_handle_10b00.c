...
int vm_handle_10b00(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_shr, sop_len) == 0)
		{
				...
				xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_data_size == INST_ITEM_SIZE_32)
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_data_size == INST_ITEM_SIZE_8)
				{
					...
					sprintf(outs + strlen(outs), "d[sp]=%llx  ", xx_contect[n].r[index]);
					...
					sprintf(outs + strlen(outs), "b[sp+4]=%llx  ", xx_contect[n].r[index]);
					...
					//sprintf(outs+strlen(outs),"%s=",tmp);
					index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					sprintf(outs + strlen(outs), "q[sp]=eflag,d[sp+4]=%llx  ", xx_contect[n + 1].r[index]);
