...
mov t0,[sp+00]
mov bt1,b[sp+4]
lea sp,[sp+fe];sub sp,2
shr t0,bt1
mov [sp+4],t0
*/
int vm_handle_0e10(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def,t0)==0)
			{
					...
					xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def,t1)==0)
				{
						...
						xreg_incmp(t2,xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def)==0)
					{
						...
						//memset(tmp,0,sizeof(tmp));
						//xx_get_reg(xx_inst[n].xx_inst_items.xx_inst_items_var[2].item_reg_def,tmp);
						//sprintf(outs+strlen(outs),"%s=",tmp);
						...
						//sprintf(outs+strlen(outs),"%s=",tmp);
						index=get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
						sprintf(outs+strlen(outs),"[sp]=eflag,[sp+4]=%08x",xx_contect[n+1].r[index]);
