...
int xx_start_execute(struct XX_CONTEXT64 *init_context, struct XX_CONTEXT64 *out_context, int *out_type);
int get_inst_context(struct XX_INST* g_inst, struct XX_CONTEXT64 *g_context, int *num,int *);
...
__declspec(noinline) int xx_start_execute(struct XX_CONTEXT64 *init_context, struct XX_CONTEXT64 *out_context, int *out_type)
{
	int iret = 0;
	...
	memset(out_context, 0, sizeof(struct XX_CONTEXT64));
	/*xx_iat这里取g_context[inst_num]*/
	memcpy(out_context, &g_context[inst_num - 1], sizeof(struct XX_CONTEXT64));
...
__declspec(noinline) int get_inst_context(struct XX_INST* g_inst, struct XX_CONTEXT64 *g_context, int *num, int *out_type)
{
	int n = 0;
	int iret = 0;
	int inst_num = 0;
	...
	while (1)
	{
		...
		*/
		if (ip<xx_mod.base || ip>(xx_mod.base + xx_mod.size))
		{
			...
			{
				memset(debug_buf, 0, sizeof(debug_buf));
				sprintf(debug_buf, "%08llx API:%s", ip, tmp);
				xx_append_file(log_file, debug_buf, strlen(debug_buf));
				break;
			}
		}
...
#if 0
		if (memcmp(g_inst[n].xx_inst_code.opcode_type, sop_nop, sop_len) == 0)
		{
			c_nop++;
		}
		...
		if (memcmp(g_inst[n].xx_inst_code.opcode_type, sop_ret, sop_len) == 0)
		{
			...
			/*64位item_const[0] == 0x08*/
			else if (g_inst[n].xx_inst_items.nitem == 1 && g_inst[n].xx_inst_items.xx_inst_items_var[0].item_const[0] == 0x08)
			{
				*out_type = 2;
			}
			//t_vm_iat[n].ret=ip;
...
执行前生成所有的内存文件
*/
__declspec(noinline) int xx_execute(struct XX_INST *inst, struct XX_CONTEXT64 *in_context, struct XX_CONTEXT64 *out_context)
{
	...
	sprintf(debug_buf, "disasm:[%s] op_type:%s-%08x  nitem:%08x", \
		xx_inst.xx_inst_mic.disasm, xx_inst.xx_inst_mic.opcode_mic, sop_type, xx_inst.xx_inst_items.nitem);
	debug_log(debug_buf);
#endif
