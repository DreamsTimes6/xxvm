...
int vm_handle_10610(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
	...
	for (n = 0; n < num; n++)
	{
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_mov, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, t0) == 0)
			{
				...
				{
					sprintf(outs + strlen(outs), "(add sp,c)  ");
					...
					index = get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					memcpy(item_data.result, &xx_contect[n].r[index], 4);
