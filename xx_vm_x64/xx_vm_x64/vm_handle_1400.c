...
int vm_handle_1400(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
	...
	for (n = 0; n < num; n++)
	{
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_call, sop_len) == 0)
		{
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
				...
				sprintf(outs + strlen(outs), "call:%llx  %s  ", xx_contect[n].r[index], tmp);
				call_index = n;
				f_cnd = 1;
				break;
			}
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_mov, sop_len) == 0)
		{
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
				...
				if (xx_inst[n].xx_inst_items.xx_inst_items_var[1].index_scale == 0x04)
				{
					...
					sprintf(outs + strlen(outs), "arg_count=%llx  ", xx_contect[n + 1].r[index]);
	...
	sprintf(outs + strlen(outs), "retn=%llx  ", xx_contect[call_index + 1].r[index]);
