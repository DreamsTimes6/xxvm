...
int vm_handle_10502(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_mov, sop_len) == 0)
		{
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0] == 0x00)
				{
					...
					sprintf(outs + strlen(outs), "(add sp,6) [sp]=%llx  ", xx_contect[n + 1].r[index]);
			...
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG)
			{
					...
					xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0] == 0x00)
				{
					if (xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_data_size == INST_ITEM_SIZE_8)
					{
						...
						sprintf(outs + strlen(outs), "[[sp]]=[%llx]=", xx_contect[n].r[index]);
	...
	for (n = 0; n < num; n++)
	{
		...
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_add, sop_len) == 0)
		{
				...
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, vm_sp) == 0)
			{
				...
				{
					/*检查第二个操作项寄存器*/
					if (xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_const[0] == 0x06)
					{
						//_Addtolist(0,1,"second condition inst_num:[%d]",n);
						t_cnd = 1;
					}
