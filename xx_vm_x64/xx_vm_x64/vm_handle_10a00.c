...
int vm_handle_10a00(struct XX_INST *xx_inst, int num, struct XX_CONTEXT64 *xx_contect, char *outs, int outs_size, char *out_var)
{
	int n = 0;
	int iret = 0;
	int f_cnd = 0;
	...
	{
		/*检查操作码*/
		if (memcmp(xx_inst[n].xx_inst_code.opcode_type, sop_div, sop_len) == 0)
		{
			/*检查第二个操作项类型*/
			if (xx_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type == INST_ITEM_TYPE_REG &&
				xreg_cmp(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def, t2) == 0)
			{
				...
				sprintf(outs + strlen(outs), "(sub sp,4) %08x / %08x = %08x ,%08x", \
					(unsigned long)xx_contect[n].r[get_reg_index(xreg_rax)], \
					(unsigned long)xx_contect[n].r[get_reg_index(t2)], \
					(unsigned long)xx_contect[n + 1].r[get_reg_index(xreg_rax)], (unsigned long)xx_contect[n + 1].r[get_reg_index(xreg_rdx)]);
				...
				memcpy(item_data.var[1], &xx_contect[n].r[get_reg_index(xreg_rax)], sizeof(xx_contect[n].r[get_reg_index(xreg_rax)]));
				memcpy(item_data.var[2], &xx_contect[n].r[get_reg_index(t2)], sizeof(xx_contect[n].r[get_reg_index(t2)]));


				memcpy(item_data.result, &xx_contect[n + 1].r[get_reg_index(xreg_rax)], sizeof(xx_contect[n + 1].r[get_reg_index(xreg_rax)]));
