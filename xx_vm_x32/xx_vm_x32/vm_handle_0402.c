...
mov bt1,b[sp+04]
add bt0,bt1
sub sp,02;lea sp,[sp+fe]
mov w[sp+4],wt0
*/
int vm_handle_0402(struct XX_INST *xx_inst,int num,struct XX_CONTEXT *xx_contect,char *outs,int outs_size,char *out_var)
{
	int n=0;
	int iret=0;
	int f_cnd=0;
		...
		if(memcmp(xx_inst[n].xx_inst_code.opcode_type,sop_add,sop_len)==0)
		{
				...
				xreg_incmp(t0,xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def)==0)
			{
					...
					xreg_incmp(t1,xx_inst[n].xx_inst_items.xx_inst_items_var[1].item_reg_def)==0)
				{
					...
					sprintf(outs+strlen(outs),"(sub sp,2) %08x ",xx_contect[n].r[index]);
					...
					//sprintf(outs+strlen(outs),"%s=",tmp);
					index=get_reg_index(xx_inst[n].xx_inst_items.xx_inst_items_var[0].item_reg_def);
					sprintf(outs+strlen(outs),"[sp+4]=%08x  ",xx_contect[n+1].r[index]);
