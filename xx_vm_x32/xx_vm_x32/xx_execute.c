...
int free_mem_file();
int xx_gen_fmem(unsigned  long addr, struct XX_FMEM *xx_fmem, int flag);
int xx_get_fmem(unsigned  long addr, char *outname, unsigned long  *out_offset);
int xx_write_fmem(unsigned  long addr, char* pdata, unsigned int datasize);
int xx_read_fmem(unsigned  long addr, char* pdata, unsigned int datasize);
int init_file_path(char *ppath, char *pname, char *file);
int xx_init32(char* mainname);
...
int calc_item(struct XX_INST *, int, struct ITEM_VAR *, struct XX_CONTEXT *, unsigned  long, int);
int conv_item(struct XX_INST *, unsigned int, int, struct XX_CONTEXT *, struct ITEM_VAR *);
...
/* vmp38 integration: memory fallback hooks (headless) */
int (*vmp38_mem_hook_read)(unsigned long addr, char *pdata, unsigned int size) = 0;
int (*vmp38_mem_hook_write)(unsigned long addr, char *pdata, unsigned int size) = 0;
...
struct XX_CONTEXT g_context[MAX_INST + 3];
//////////////////////////////////////////////////////////////////////
...
extern int vm_handle(struct XX_HTHREAD *x_thread32);
...
分析函数中的信号必须释放
*/
__declspec(noinline) int xx_start_saly()
{
	int iret = 0;
...
__declspec(noinline) void xx_saly_stop()
{
	int iret = 0;
...
__declspec(noinline) void xx_saly_semaphore(HANDLE sem)
{
	saly_semaphore = sem;
}
...
/**/
__declspec(noinline) int is_vm_entry()
{
	int iret = 0;
	int n = 0;
	int inst_num = 0;
	...
	while (1)
	{
		...
		if (g_inst[n].xx_inst_exist.finish_flag != INST_FLAG_EXIST)
		{
			...
			memset(debug_buf, 0, sizeof(debug_buf));
			sprintf(debug_buf, "xx_disasm error.inst_system:[%x] base:[%lx] ip:[%lx]", \
				g_inst[n].xx_inst_code.inst_system, \
				*(ulong*)g_inst[n].xx_inst_code.base, \
				*(ulong*)g_inst[n].xx_inst_code.current_addr);
			xx_append_file(log_file, debug_buf, strlen(debug_buf));
			return 0;
		}
		...
		{
			ip = *(ulong*)g_inst[n].xx_inst_items.xx_inst_items_var[0].item_const;
...
__declspec(noinline) int xx_start_execute(struct XX_CONTEXT *init_context)
{
	int iret = 0;
	struct XX_CONTEXT in_context;
...
/*测试*/
		...
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "sp:[%s] opcode:[%s] ip:[%s] ", buf1, buf2, buf3);
		xx_append_file(log_file, debug_buf, strlen(debug_buf));

		memset(buf1, 0, sizeof(buf1));
		xx_get_reg_mic(vm_key, buf1);
		memset(buf2, 0, sizeof(buf2));
		xx_get_reg_mic(vm_regbase, buf2);
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "vm_key:[%s] vm_regbase:[%s]\r\n", buf1, buf2);
		xx_append_file(log_file, debug_buf, strlen(debug_buf));
...
__declspec(noinline) int get_inst_context(struct XX_INST* g_inst, struct XX_CONTEXT *g_context, int *num)
{
	int n = 0;
	int iret = 0;
	int inst_num = 0;
	...
	while (1)
	{
		...
		*/
		if (ip<xx_mod.base || ip>(xx_mod.base + xx_mod.size))
		{
			...
			{
				memset(debug_buf, 0, sizeof(debug_buf));
				sprintf(debug_buf, "%08x API:%s", ip, tmp);
				xx_append_file(log_file, debug_buf, strlen(debug_buf));
				return 0;
			}
		}
		...
		if (iret == 0)
		{
			...
			memset(debug_buf, 0, sizeof(debug_buf));
			sprintf(debug_buf, "ip:[%lx]  %s", g_context[n].ip, "xx_execute error\r\n");
			xx_append_file(log_file, debug_buf, strlen(debug_buf));
		...
		if (memcmp(g_inst[n].xx_inst_code.opcode_type, sop_jmp, sop_len) == 0 && \
			g_inst[n].xx_inst_items.xx_inst_items_flag[0].item_type != INST_ITEM_TYPE_REG)
		{
			memcpy(&g_context[n], &g_context[n + 1], sizeof(struct XX_CONTEXT));
			continue;
		}
		...
		if (n >= MAX_INST)
		{
			//memcpy(&g_context[0], &g_context[n - 1], sizeof(struct XX_CONTEXT));
			memcpy(&g_context[0], &g_context[n], sizeof(struct XX_CONTEXT));
			n = 0;
			inst_num = 0;
		}
	}
...
int get_inst_context_test(struct XX_CONTEXT *in_context, struct XX_CONTEXT *out_context)
{
	int n = 0;
	int iret = 0;
	int inst_num = 0;
	...
	if (xx_inst.xx_inst_exist.finish_flag != INST_FLAG_EXIST)
	{
		...
		sprintf(debug_buf, "xx_disasm error.inst_system:[%x] base:[%lx] ip:[%lx]", \
			...
			*(ulong*)xx_inst.xx_inst_code.base, \
			*(ulong*)xx_inst.xx_inst_code.current_addr);
		xx_append_file(log_file, debug_buf, strlen(debug_buf));
		return 0;
	}
#if 1
	...
	if (iret == 0)
	{
		//debug_log("xx_execute2 error"); 
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "ip:[%lx]  %s", ip, "xx_execute error\r\n");
		xx_append_file(log_file, debug_buf, strlen(debug_buf));
		return 0;
	}
#endif
...
int xx_execute(struct XX_INST *inst, struct XX_CONTEXT *in_context, struct XX_CONTEXT *out_context)
{
	...
	if (xx_link == 0)
	{
		/* lazy init: xx_link is normally created by xx_start_execute, but
		   vmp38_exec_step_xx calls xx_execute directly without it - create
		   the node list on first use so the engine actually runs instead of
		   failing every instruction (this was why the static restore fell
		   back to mini_exec for every step). */
		xx_link = xx_list_init();
	}

#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "rax:%lx rcx:%lx rdx:%lx rbx:%lx rsp:%lx rbp:%lx rsi:%lx rdi:%lx ip:%lx rflag:%lx", \
...
#ifdef DEBUG_LOG
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "item[%d]: addr=%lx data=%lx %lx ", \
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "rax:%lx rcx:%lx rdx:%lx rbx:%lx rsp:%lx rbp:%lx rsi:%lx rdi:%lx rip:%lx rflag:%lx", \
...
int calc_item(struct XX_INST *inst, int nitem, struct ITEM_VAR *out_var, \
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "xx_read_fmem(%lx, %08x )", addr, datasize);
	debug_log(debug_buf);
#endif
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "xx_read_file(%s,%lx,%08x )", filename, foffset, datasize);
	debug_log(debug_buf);
#endif
	/*从文件中读取数据*/
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "xx_write_fmem(%lx, %08x )", addr, datasize);
	debug_log(debug_buf);
#endif
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "addr:[%lx] xx_scover_file(%s,%lx,%08x )", addr, filename, foffset, datasize);
	debug_log(debug_buf);
#endif
...
#ifdef DEBUG_LOG
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "var_fmem->start:%lx var_fmem->end:%lx var_fmem->filename:%s offset:%lx", \
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "buf_fmem.start:%lx buf_fmem.end:%lx buf_fmem.filename:%s offset:%lx", \
...
int xx_gen_fmem(unsigned long  addr, struct XX_FMEM *xx_fmem, int flag)
{
...
#ifdef DEBUG_LOG
		memset(debug_buf, 0, sizeof(debug_buf));
		sprintf(debug_buf, "addr:[%lx]", addr);
		debug_log(debug_buf);
#endif
		//memset(debug_buf,0,sizeof(debug_buf));
	...
	if (flag == 0)
	{
		...
		{
			xx_fmem->start = (unsigned  long)meminfo.BaseAddress;
			xx_fmem->end = ((unsigned  long)meminfo.BaseAddress + meminfo.RegionSize);
			//debug_log("VirtualQueryEx");
			return 0;
		}
	}
...
#ifdef DEBUG_LOG
	memset(debug_buf, 0, sizeof(debug_buf));
	sprintf(debug_buf, "addr:%lx meminfo.AllocationBase:%lx meminfo.BaseAddress:%lx meminfo.RegionSize:%lx ", \
		addr, (unsigned  long)meminfo.AllocationBase, (unsigned  long)meminfo.BaseAddress, (unsigned  long) meminfo.RegionSize);
	debug_log(debug_buf);
#endif
	...
	iret = DbgMemRead((unsigned  long)meminfo.BaseAddress, buf, meminfo.RegionSize);
	if (iret == 0)
	{
		free(buf);
	...
	/*生成文件名，赋值xx_fmem*/
	memset(tmp, 0, sizeof(tmp));
	sprintf(tmp, "%s%lx-%lx.x", fmempath, (unsigned  long)meminfo.BaseAddress, \
		((unsigned  long)meminfo.BaseAddress + meminfo.RegionSize));
...
int xdbg_gen_fmem()
{
	...
	for (n = 0; n < mmap.count; n++)
	{
		...
		iret = DbgMemRead((unsigned  long)meminfo->BaseAddress, tset_buf, 10);
		if (iret == 0)
		{
			/*尝试读取失败，继续下一个*/
			continue;
		...
		xx_list_insert(xx_link, &xx_fmem, sizeof(struct XX_FMEM));
	}
