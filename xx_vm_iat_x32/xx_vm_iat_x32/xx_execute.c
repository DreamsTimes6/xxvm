...
int xx_init(char* mainname);
int debug_log(char *tmp);
int xx_start_execute(struct XX_CONTEXT *init_context, struct XX_CONTEXT *out_context, int *out_type);
int get_inst_context(struct XX_INST* g_inst, struct XX_CONTEXT *g_context, int *num, int *);
...
__declspec(noinline) int xx_start_execute(struct XX_CONTEXT *init_context, struct XX_CONTEXT *out_context, int *out_type)
{
	int iret = 0;
...
__declspec(noinline) int get_inst_context(struct XX_INST* g_inst, struct XX_CONTEXT *g_context, int *num, int *out_type)
{
	int n = 0;
	int iret = 0;
	int inst_num = 0;
	...
	while (1)
	{
		...
		*/
...
#if 0
		if (memcmp(g_inst[n].xx_inst_code.opcode_type, sop_nop, op_len) == 0)
		{
			c_nop++;
		}
		if (inst_num > 8 && c_nop != 1)
		{
			return 0;
		}
#endif
		...
		{
			//debug_log("not vm");
			if (g_inst[n].xx_inst_items.nitem == 0)
			{
				*out_type = 1;
			}
			else if (g_inst[n].xx_inst_items.nitem == 1 && g_inst[n].xx_inst_items.xx_inst_items_var[0].item_const[0] == 0x04)
			{
				*out_type = 2;
			}
			//t_vm_iat[n].ret=ip;
...
__declspec(noinline) int get_inst_context_test(struct XX_CONTEXT *in_context, struct XX_CONTEXT *out_context)
{
	int n = 0;
	int iret = 0;
	int inst_num = 0;
...
执行前生成所有的内存文件
*/
__declspec(noinline) int xx_execute(struct XX_INST *inst, struct XX_CONTEXT *in_context, struct XX_CONTEXT *out_context)
{
