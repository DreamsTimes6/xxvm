...
extc int     cdecl _Tablefunction(t_table *pt,
...
extc int     cdecl _Insertname(ulong addr,int type,char *name);
...
extc int     cdecl _Findname(ulong addr,int type,char *name);
...
extc int     cdecl _Findsymbolicname(ulong addr,char *fname);
...
extc ulong   cdecl Disassembleforward(uchar *block,ulong base,ulong size,
               ulong ip,int n,int usedec);
extc int     cdecl _Issuspicious(char *cmd,ulong size,ulong ip,
...
extc int     cdecl _Assemble(char *cmd,ulong ip,t_asmmodel *model,int attempt,
...
extc HWND    cdecl _Createthreadwindow(void);
extc t_thread* cdecl _Findthread(ulong threadid);
extc int     cdecl Decodethreadname(char *s,ulong threadid,int mode);
extc ulong   cdecl _Getcputhreadid(void);
...
extc ulong   cdecl _Readcommand(ulong ip,char *cmd);
...
extc HWND    cdecl _Quicktablewindow(t_table *pt,int nlines,int maxcolumns,
               char *winclass,char *wintitle);
...
extc int     cdecl _Setbreakpointext(ulong addr,ulong type,char cmd,
               ulong passcount);
...
extc int     cdecl _Sethardwarebreakpoint(ulong addr,int size,int type);
extc int     cdecl _Deletehardwarebreakpoint(int index);
extc int     cdecl _Deletehardwarebreakbyaddr(ulong addr);
...
extc HWND    cdecl _Creatertracewindow(void);
extc void    cdecl _Settracecondition(char *cond,int onsuspicious,
...
extc int     cdecl _Startruntrace(t_reg *preg);
extc void    cdecl _Deleteruntrace(void);
extc int     cdecl _Runtracesize(void);
extc int     cdecl _Findprevruntraceip(ulong ip,int startback);
extc int     cdecl Findnextruntraceip(ulong ip,int startback);
extc int     cdecl _Getruntraceregisters(int nback,t_reg *preg,
...
extc void    cdecl _Sendshortcut(int where,ulong addr,
...
extc int     cdecl _Go(ulong threadid,ulong tilladdr,int stepmode,
...
extc int     cdecl _Registerpluginclass(char *classname,char *iconname,
...
extc int     cdecl _Pluginreadstringfromini(HINSTANCE dllinst,char *key,
               char *s,char *def);
