...
char debug_buf[500];
////////////////////////////////////////////////
int get_inst_context(struct XX_INST* g_inst,struct XX_CONTEXT *g_context,int *num,int *out_type);
...
struct XX_INST g_inst[MAX_INST+1];
struct XX_CONTEXT g_context[MAX_INST+2];
...
int xx_start_execute(struct XX_CONTEXT *init_context,struct XX_CONTEXT *out_context,int *out_type)
{
	...
	sprintf(tmp,"inst_num:[%08x] ",inst_num);
	xx_append_file("11.txt",tmp,strlen(tmp));
	for(n=0;n<inst_num;n++)
	{
		memset(tmp,0,sizeof(tmp));
		sprintf(tmp,"[%08x] ip[%08x] ",n,g_context[n].ip);
		xx_append_file("11.txt",tmp,strlen(tmp));
	}
...
int xx_gen_fmem(unsigned int addr,struct XX_FMEM *xx_fmem,int flag)
{
	...
	memset(buf,0,sizeof(meminfo.RegionSize));
