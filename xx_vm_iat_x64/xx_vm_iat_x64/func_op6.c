...
int func_op_movsb(struct XX_INST *xx_inst,struct ITEM_VAR64 *var,struct XX_CONTEXT64 *in_context,struct XX_CONTEXT64 *out_context)
{
	int iret=0;
	int n=0;
	ulong64 src=0;
	...
	{
		var_prefix=xx_inst->xx_inst_code.prefix[n];
		if(var_prefix==(unsigned char)0xf3)
		{
			size=(unsigned int)in_context->r[REG_RCX];
			break;
		}
	}
	...
	/*数据地址  df标志*/
	src=in_context->r[REG_RSI];
	des=in_context->r[REG_RDI];
	if((eflag & 0x00000400)==0x00000400)
	{
		src=src-size-num;
		des=des-size-num;
	}
	...
	/*读取数据*/
	iret=xx_read_fmem(src,buf,size);
	if(iret==0)
	{
		free(buf);
		return 0;
	...
	/*写入数据*/
	iret=xx_write_fmem(des,buf,size);
	if(iret==0)
	{
		free(buf);
		return 0;
	...
	if((eflag & 0x00000400)==0x00000400)
	{
		out_context->r[REG_RSI]=src;
		out_context->r[REG_RDI]=des;
	}
	else
	{
		out_context->r[REG_RSI]=src+size;
		out_context->r[REG_RDI]=des+size;
	}
...
int func_op_movsd(struct XX_INST *xx_inst,struct ITEM_VAR64 *var,struct XX_CONTEXT64 *in_context,struct XX_CONTEXT64 *out_context)
{
	int iret=0;
	int n=0;
	ulong64 src=0;
	...
	{
		var_prefix=xx_inst->xx_inst_code.prefix[n];
		if(var_prefix==(unsigned char)0x66)
		{
			num=2;
			break;
		}
